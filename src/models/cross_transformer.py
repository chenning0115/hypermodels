...
class ABTest:
    sqsformer = 0
    without_center_and_fusion = 1
    without_fusion = 2
    without_rotation = 3
    without_position_embedding = 4
    without_rotation_and_embedding = 5
    without_all = 6
...
class HSINet(nn.Module):
    def __init__(self, params):
        ...
        if  self.model_type == ABTest.without_all:
            self.new_image_size = image_size + 1
        else:
    ...
    def encoder_block(self, x):
        ...
        #local transformer
        if self.model_type == ABTest.without_center_and_fusion:
            # 去掉中心token
            pos_emb = self.get_position_embedding(x_pixel, (h//2, w//2), cls_token=True)
        ...
        elif self.model_type == ABTest.sqsformer or self.model_type == ABTest.without_fusion or self.model_type == ABTest.without_rotation:
            # SQSFormer
            ...
            if self.model_type == ABTest.without_fusion:
                # 不使用fusion 而只用最后一层的center pixel
                logit_pixel = x_center_list[-1]
            else:
                # 使用fusion
                logit_pixel = torch.sum(x_center_tensor * self.center_weight, dim=0)
        elif self.model_type == ABTest.without_position_embedding or self.model_type == ABTest.without_rotation_and_embedding:
            # remote postion embedding
            x_pixel = rearrange(x_pixel, 'b s w h-> b (w h) s') # (batch, w*h, s)
            ...
            x_center_tensor = torch.stack(x_center_list, dim=0) # [depth, batch, dim] 
            logit_pixel = torch.sum(x_center_tensor * self.center_weight, dim=0)
        elif self.model_type == ABTest.without_all:
            x_pixel = rearrange(x_pixel, 'b s w h-> b (w h) s') # (batch, w*h, s)
            cls_tokens_pixel = self.cls_token_pixel.expand(x_pixel.shape[0], -1, -1)
            x_pixel = torch.cat((cls_tokens_pixel, x_pixel), dim = 1) #[b,image+1,dim]
            x_pixel = x_pixel + torch.unsqueeze(self.pixel_pos_embedding[:,:],0) * self.pixel_pos_scale
