...
class HSINet(nn.Module):
    def __init__(self, params):
        ...
        self.conv2d_features = nn.Sequential(
            nn.Conv2d(in_channels=self.spectral_size, out_channels=conv2d_out, kernel_size=(5, 5), padding=(2,2)),
        ...
        self.conv2d_features1 = nn.Sequential(
            nn.Conv2d(in_channels=self.spectral_size, out_channels=conv2d_out, kernel_size=(1, 1), padding=(0, 0)),
            nn.BatchNorm2d(conv2d_out),
            nn.ReLU(),
            # featuremap 是在这之后加一层channel上的压缩
            # nn.Conv2d(in_channels=conv2d_out,out_channels=dim,kernel_size=3,padding=1),
        ...
        input_linear_dim = dim
        linear_dim = input_linear_dim * 2
        self.classifier_mlp = nn.Sequential(
            nn.Linear(input_linear_dim, linear_dim),
    ...
    def encoder_block(self, x):
        ...
        # print(x_pixel.shape)
        # cur = int((self.path_size * self.path_size + 1) / 2 )
        # logit_pixel = self.to_latent_pixel(x_pixel[:,cur,:])

        # add 方式
        # logit_pixel = x_center_list[0]
        # for i in range(1, len(x_center_list)):
            # logit_pixel = torch.add(logit_pixel, x_center_list[i])
