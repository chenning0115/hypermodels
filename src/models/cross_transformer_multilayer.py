...
class HSINet(nn.Module):
    def __init__(self, params):
        ...
        # self.pixel_pos_embedding = nn.Parameter(torch.randn(1, self.new_image_size, dim))
        ...
        self.selected_patch_size = [13]
        self.weight_patch = nn.Parameter(torch.ones(len(self.selected_patch_size), 1, 1))
    ...
    def encoder_block(self, x):
        ...
        # _, channel, _, _ = x_pixel.shape
        # center_h = h // 2
        # center_w = w // 2
        # token_list = [x_pixel[:,:,center_h, center_w]]
        # for cur_patch in self.selected_patch_size:
        #     temp = cur_patch // 2
        #     start_h, start_w = center_h - temp, center_w - temp
        #     temp_img = x_pixel[:,:,start_w:start_w+cur_patch, start_h:start_h+cur_patch]
        #     # print(temp_img.shape)
        #     temp_img = torch.mean(temp_img.reshape((b,channel,cur_patch * cur_patch)), -1)
        #     token_list.append(temp_img)
        # x_pixel = torch.stack(token_list, dim=1)
        #2. patch_embedding
        ...
        logit_x = self.classifier_mlp(logit_x)
    ...
    def forward(self, x,left=None,right=None):
        ...
        center_h = h // 2
        center_w = w // 2
        # token_list = [x[:,:,center_h, center_w]]
        token_list = []
        for cur_patch in self.selected_patch_size:
            temp = cur_patch // 2
            start_h, start_w = center_h - temp, center_w - temp
            temp_img = x[:,:,start_w:start_w+cur_patch, start_h:start_h+cur_patch]
            logit_x, _ = self.encoder_block(temp_img)
            token_list.append(logit_x)
        tokens = torch.stack(token_list, dim=0)    
        logit_x = torch.sum(tokens * self.weight_patch, dim=0)
